...
{
    QString ret = QString("RGCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non RGC unit input in lnReqAmount, we generate the URI with RGC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(namocoinUnits::format(namocoinUnits::RGC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
