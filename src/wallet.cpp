...
// RGCoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-RGCoin-address
...
// RGCoin: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CnamocoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CnamocoinAddress(address).ToString());
}
...
// RGCoin: check 'spent' consistency between wallet and txindex
// RGCoin: fix wallet spent state according to txindex
...
// RGCoin: disable transaction (only for coinstake)
